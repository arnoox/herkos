...
fn test_trap_on_zero() {
    assert!(unreachable::func_1(0).is_err(), "should trap when n == 0");
}

#[test]
fn test_no_trap_on_nonzero() {
    assert_eq!(unreachable::func_1(5).unwrap(), 5);
    assert_eq!(unreachable::func_1(1).unwrap(), 1);
    assert_eq!(unreachable::func_1(-1).unwrap(), -1);
}
...
fn test_safe_div_normal() {
    assert_eq!(unreachable::func_2(10, 3).unwrap(), 3);
    assert_eq!(unreachable::func_2(100, 10).unwrap(), 10);
    assert_eq!(unreachable::func_2(-10, 3).unwrap(), -3);
}
...
fn test_safe_div_traps_on_zero() {
    assert!(
        unreachable::func_2(10, 0).is_err(),
...
fn test_dead_unreachable() {
    assert_eq!(
        unreachable::func_3(42).unwrap(),
        42,
        "dead unreachable after return should not execute"
    );
    assert_eq!(unreachable::func_3(0).unwrap(), 0);
    assert_eq!(unreachable::func_3(-7).unwrap(), -7);
}
