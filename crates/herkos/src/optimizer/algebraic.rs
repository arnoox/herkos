...
fn constant_operand_rule(
    ...
    match op {
        ...
        BinOp::I32Mul => match (lhs_val, rhs_val) {
            ...
            (_, Some(IrValue::I32(0))) | (Some(IrValue::I32(0)), _) => {
                Some(IrInstr::Const { dest, value: IrValue::I32(0) })
            }
            _ => None,
        },
        BinOp::I64Mul => match (lhs_val, rhs_val) {
            ...
            (_, Some(IrValue::I64(0))) => Some(IrInstr::Const { dest, value: IrValue::I64(0) }),
            (Some(IrValue::I64(0)), _) => Some(IrInstr::Const { dest, value: IrValue::I64(0) }),
            _ => None,
        },
        ...
        BinOp::I64And => match (lhs_val, rhs_val) {
            (_, Some(IrValue::I64(0))) | (Some(IrValue::I64(0)), _) => {
                Some(IrInstr::Const { dest, value: IrValue::I64(0) })
            }
            (_, Some(IrValue::I64(-1))) => Some(IrInstr::Assign { dest, src: lhs }),
            (Some(IrValue::I64(-1)), _) => Some(IrInstr::Assign { dest, src: rhs }),
            _ => None,
        ...
        BinOp::I32Or => match (lhs_val, rhs_val) {
            (_, Some(IrValue::I32(0))) => Some(IrInstr::Assign { dest, src: lhs }),
            (Some(IrValue::I32(0)), _) => Some(IrInstr::Assign { dest, src: rhs }),
            (_, Some(IrValue::I32(-1))) | (Some(IrValue::I32(-1)), _) => {
                Some(IrInstr::Const { dest, value: IrValue::I32(-1) })
            }
            _ => None,
        },
        BinOp::I64Or => match (lhs_val, rhs_val) {
            (_, Some(IrValue::I64(0))) => Some(IrInstr::Assign { dest, src: lhs }),
            (Some(IrValue::I64(0)), _) => Some(IrInstr::Assign { dest, src: rhs }),
            (_, Some(IrValue::I64(-1))) | (Some(IrValue::I64(-1)), _) => {
                Some(IrInstr::Const { dest, value: IrValue::I64(-1) })
            }
            _ => None,
        },
...
mod tests {
    ...
    fn add_zero_rhs() {
        ...
        let mut func = make_func(single_block(vec![
            IrInstr::Const { dest: VarId(1), value: IrValue::I32(0) },
            IrInstr::BinOp { dest: VarId(2), op: BinOp::I32Add, lhs: VarId(0), rhs: VarId(1) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[1], IrInstr::Assign { dest: VarId(2), src: VarId(0) }));
    }
    ...
    fn add_zero_lhs() {
        ...
        let mut func = make_func(single_block(vec![
            IrInstr::Const { dest: VarId(1), value: IrValue::I32(0) },
            IrInstr::BinOp { dest: VarId(2), op: BinOp::I32Add, lhs: VarId(1), rhs: VarId(0) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[1], IrInstr::Assign { dest: VarId(2), src: VarId(0) }));
    }
    ...
    fn mul_one() {
        let mut func = make_func(single_block(vec![
            ...
            IrInstr::BinOp { dest: VarId(2), op: BinOp::I32Mul, lhs: VarId(0), rhs: VarId(1) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[1], IrInstr::Assign { dest: VarId(2), src: VarId(0) }));
    }
    ...
    fn mul_zero() {
        ...
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[1], IrInstr::Const { dest: VarId(2), value: IrValue::I32(0) }));
    }
    ...
    fn xor_self() {
        let mut func = make_func(single_block(vec![
            IrInstr::BinOp { dest: VarId(1), op: BinOp::I32Xor, lhs: VarId(0), rhs: VarId(0) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[0], IrInstr::Const { dest: VarId(1), value: IrValue::I32(0) }));
    }
    ...
    fn eq_self() {
        let mut func = make_func(single_block(vec![
            IrInstr::BinOp { dest: VarId(1), op: BinOp::I32Eq, lhs: VarId(0), rhs: VarId(0) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[0], IrInstr::Const { dest: VarId(1), value: IrValue::I32(1) }));
    }

    #[test]
    fn ne_self() {
        let mut func = make_func(single_block(vec![
            IrInstr::BinOp { dest: VarId(1), op: BinOp::I32Ne, lhs: VarId(0), rhs: VarId(0) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[0], IrInstr::Const { dest: VarId(1), value: IrValue::I32(0) }));
    }
    ...
    fn and_zero() {
        let mut func = make_func(single_block(vec![
            IrInstr::Const { dest: VarId(1), value: IrValue::I32(0) },
            IrInstr::BinOp { dest: VarId(2), op: BinOp::I32And, lhs: VarId(0), rhs: VarId(1) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[1], IrInstr::Const { dest: VarId(2), value: IrValue::I32(0) }));
    }
    ...
    fn or_all_ones() {
        let mut func = make_func(single_block(vec![
            IrInstr::Const { dest: VarId(1), value: IrValue::I32(-1) },
            IrInstr::BinOp { dest: VarId(2), op: BinOp::I32Or, lhs: VarId(0), rhs: VarId(1) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[1], IrInstr::Const { dest: VarId(2), value: IrValue::I32(-1) }));
    }
    ...
    fn shl_zero() {
        let mut func = make_func(single_block(vec![
            IrInstr::Const { dest: VarId(1), value: IrValue::I32(0) },
            IrInstr::BinOp { dest: VarId(2), op: BinOp::I32Shl, lhs: VarId(0), rhs: VarId(1) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[1], IrInstr::Assign { dest: VarId(2), src: VarId(0) }));
    }
    ...
    fn sub_self() {
        let mut func = make_func(single_block(vec![
            IrInstr::BinOp { dest: VarId(1), op: BinOp::I32Sub, lhs: VarId(0), rhs: VarId(0) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[0], IrInstr::Const { dest: VarId(1), value: IrValue::I32(0) }));
    }
    ...
    fn sub_zero() {
        let mut func = make_func(single_block(vec![
            IrInstr::Const { dest: VarId(1), value: IrValue::I32(0) },
            IrInstr::BinOp { dest: VarId(2), op: BinOp::I32Sub, lhs: VarId(0), rhs: VarId(1) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[1], IrInstr::Assign { dest: VarId(2), src: VarId(0) }));
    }
    ...
    fn i64_add_zero() {
        let mut func = make_func(single_block(vec![
            IrInstr::Const { dest: VarId(1), value: IrValue::I64(0) },
            IrInstr::BinOp { dest: VarId(2), op: BinOp::I64Add, lhs: VarId(0), rhs: VarId(1) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[1], IrInstr::Assign { dest: VarId(2), src: VarId(0) }));
    }
    ...
    fn i64_xor_self() {
        let mut func = make_func(single_block(vec![
            IrInstr::BinOp { dest: VarId(1), op: BinOp::I64Xor, lhs: VarId(0), rhs: VarId(0) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[0], IrInstr::Const { dest: VarId(1), value: IrValue::I64(0) }));
    }
    ...
    fn cross_block_const_simplification() {
        ...
        let mut func = make_func(vec![
            IrBlock {
                id: BlockId(0),
                instructions: vec![IrInstr::Const { dest: VarId(1), value: IrValue::I32(0) }],
                terminator: IrTerminator::Jump { target: BlockId(1) },
            },
            IrBlock {
                id: BlockId(1),
                instructions: vec![IrInstr::BinOp {
                    dest: VarId(2), op: BinOp::I32Add, lhs: VarId(0), rhs: VarId(1),
                }],
                terminator: IrTerminator::Return { value: Some(VarId(2)) },
            },
        ]);
        eliminate(&mut func);
        assert!(matches!(func.blocks[1].instructions[0], IrInstr::Assign { dest: VarId(2), src: VarId(0) }));
    }
    ...
    fn add_nonzero_unchanged() {
        let mut func = make_func(single_block(vec![
            IrInstr::Const { dest: VarId(1), value: IrValue::I32(5) },
            IrInstr::BinOp { dest: VarId(2), op: BinOp::I32Add, lhs: VarId(0), rhs: VarId(1) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[1], IrInstr::BinOp { .. }));
    }
    ...
    fn float_add_zero_unchanged() {
        let mut func = make_func(single_block(vec![
            IrInstr::Const { dest: VarId(1), value: IrValue::F32(0.0) },
            IrInstr::BinOp { dest: VarId(2), op: BinOp::F32Add, lhs: VarId(0), rhs: VarId(1) },
        ]));
        eliminate(&mut func);
    ...
    fn and_self() {
        let mut func = make_func(single_block(vec![
            IrInstr::BinOp { dest: VarId(1), op: BinOp::I32And, lhs: VarId(0), rhs: VarId(0) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[0], IrInstr::Assign { dest: VarId(1), src: VarId(0) }));
    }

    #[test]
    fn or_self() {
        let mut func = make_func(single_block(vec![
            IrInstr::BinOp { dest: VarId(1), op: BinOp::I32Or, lhs: VarId(0), rhs: VarId(0) },
        ]));
        eliminate(&mut func);
        assert!(matches!(func.blocks[0].instructions[0], IrInstr::Assign { dest: VarId(1), src: VarId(0) }));
    }
